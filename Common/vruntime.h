...
//
//		
//		if(EndSOLLoopIteration())
//			break;
//	}
//		EndSOLLoopCondition();//forgetting this will cause a memory leak
//	// always on looping SOL conditions:
//	return false;
//		
//} 
void CRunObject::StartSOLLoopCondition()
{
	EventParametersInfo2 newepi;
	epis.push_back(newepi);
     pRuntime->GetCurrentEventPtrs(epis.back());
}

void CRunObject::StartSOLLoopIteration()
{
     pRuntime->NewSOLCopy();
}

bool CRunObject::EndSOLLoopIteration()
{
     if (epis.back().pCnds && !pRuntime->RunConditions(epis.back().pCnds))
	 {
          pRuntime->RestoreSOL();
          return true;
     }
     ...
     if (epis.back().pActs)
          pRuntime->RunActions(epis.back().pActs);

     if (epis.back().pSubEvents)
          pRuntime->RunSubEvents(epis.back().pSubEvents);

     pRuntime->RestoreSOL();
     return false;
}

void CRunObject::EndSOLLoopCondition()
{
