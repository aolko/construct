...
public:
		...
		)
	{
		...
		{
			if( dwOptFlags & MDCOPT_DESKTOP_COMPATIBILITY )
				m_bMemDC = TRUE;
			else
		...
		if( m_bMemDC ) 
		{
			...
			if( (dwOptFlags & MDCOPT_NO_COPY_OPT) == 0 )
			{
				...
				if( (dwLayout&LAYOUT_RTL) != 0 )
					__EXT_MFC_SwapLeftRight( m_rect );
				pDC->LPtoDP( &m_rect );
			} // if( (dwOptFlags & MDCOPT_NO_COPY_OPT) == 0 )
			...
			else if( dwOptFlags & MDCOPT_FORCE_DIB )
			{
				...
				bih.biWidth = ( dwOptFlags & MDCOPT_DIB_NEGATIVE_WIDTH ) ? (-m_rect.Width()) : m_rect.Width();
				bih.biHeight = ( dwOptFlags & MDCOPT_DIB_NEGATIVE_HEIGHT ) ? (-m_rect.Height()) : m_rect.Height();
				bih.biPlanes = 1;
				bih.biBitCount = 32;
				bih.biCompression = BI_RGB;
				bih.biSizeImage = m_rect.Width()*m_rect.Height();
