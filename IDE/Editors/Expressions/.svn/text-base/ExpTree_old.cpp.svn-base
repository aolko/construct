...
ExpPart*& ExpPart::GetPLeftOperand()
{
	...
	static int nullity = 0;
	return (ExpPart*&)(nullity);
}
...
bool ExpIdent::Constant()
{
	...
	// Return false; no way of telling if the extension will return a constant value
	// or not.
	return false;
}
...
void ExpPower::Evaluate(ExpReturn* er)
{
	...
	case EXPTYPE_INTEGER:
		...
		case EXPTYPE_FLOAT:
			er->eType = EXPTYPE_FLOAT;
			er->eData.iVal = pow(double(er->eData.iVal), param.eData.fVal);
			return;
		}//switch
...
void ExpSin::Evaluate(ExpReturn* er)
{
	...
	switch (er->eType) {
	case EXPTYPE_INTEGER:
		er->eData.fVal = sin(DEG_TO_RAD(er->eData.iVal));
		er->eType = EXPTYPE_FLOAT;
		return;
	case EXPTYPE_FLOAT:
		er->eData.fVal = sin(DEG_TO_RAD(er->eData.fVal));
		return;
	}
...
void ExpCos::Evaluate(ExpReturn* er)
{
	...
	switch (er->eType) {
	case EXPTYPE_INTEGER:
		er->eData.fVal = cos(DEG_TO_RAD(er->eData.iVal));
		er->eType = EXPTYPE_FLOAT;
		return;
	case EXPTYPE_FLOAT:
		er->eData.fVal = cos(DEG_TO_RAD(er->eData.fVal));
		return;
	}
...
void ExpSqrt::Evaluate(ExpReturn* er)
{
	...
	case EXPTYPE_INTEGER:
		...
		return;
	case EXPTYPE_FLOAT:
		er->eData.fVal = sqrt((double)er->eData.fVal);
		return;
	}
