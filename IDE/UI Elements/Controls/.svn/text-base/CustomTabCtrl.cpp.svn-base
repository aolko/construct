...
{
	COLORREF bgColor = g_PaintManager->GetColor((m_fSelected||m_fHighlighted) ? COLOR_WINDOW     : COLOR_BTNHIGHLIGHT);
	COLORREF fgColor = g_PaintManager->GetColor((m_fSelected ||m_fHighlighted) ? COLOR_WINDOWTEXT : COLOR_BTNTEXT);

	CBrush brush(bgColor);

	CPen blackPen(PS_SOLID, 1, g_PaintManager->GetColor(COLOR_BTNTEXT));
	CPen shadowPen(PS_SOLID, 1, g_PaintManager->GetColor(COLOR_3DSHADOW));
...
LOGFONT CCustomTabCtrl::lf_default = {12, 0, 0, 0, FW_NORMAL, 0, 0, 0,
			...
			CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_DONTCARE, "Microsoft Sans Serif"};
...
void CCustomTabCtrl::OnPaint()
{
	...
	if(!m_hBmpBkLeftSpin)
	{
		m_rgbGlyph[0] = g_PaintManager->GetColor(COLOR_BTNTEXT);
		m_rgbGlyph[1] = g_PaintManager->GetColor(COLOR_BTNTEXT);
		m_rgbGlyph[2] = g_PaintManager->GetColor(COLOR_BTNTEXT);
		m_rgbGlyph[3] = g_PaintManager->GetColor(COLOR_BTNTEXT);
	}
	...
	//dcMem.FillSolidRect(&rCl,g_PaintManager->GetColor(COLOR_BTNFACE));
