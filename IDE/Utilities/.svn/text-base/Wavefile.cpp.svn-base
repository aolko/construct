...
{  
    ...
    m_bIsXWMA = false;  
    m_aDecodedPacketCumulativeBytes = NULL;  
}  
...
{  
    ...
    {  
        ...
        if (m_bIsXWMA)  
        {  
            SAFE_DELETE_ARRAY( m_aDecodedPacketCumulativeBytes );  
        }  
    }  
}  
...
//-----------------------------------------------------------------------------  
HRESULT CWaveFile::Open( LPSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags, bool asResource)  
{  
    HRESULT hr;  
    ...
    {  
        if( strFileName == NULL && !asResource)  
            return E_INVALIDARG;  
        SAFE_DELETE_ARRAY( m_pwfx );  
        ...
        {  
            ...
            if( NULL == ( hResInfo = FindResource( NULL, strFileName, "FILES" ) ) )  
            {  
...
{  
    ...
    if ( m_ckRiff.fccType != mmioFOURCC( 'W', 'A', 'V', 'E' ) )  
        m_bIsXWMA = true;  
    else 
        m_bIsXWMA = false;  
    ...
    {  
        m_pwfx = ( WAVEFORMATEX* )new CHAR[ sizeof( GENERICWAVEFILE ) ];  
        if( NULL == m_pwfx )  
    ...
    {  
        ...
        m_pwfx = ( WAVEFORMATEX* )new CHAR[ sizeof( GENERICWAVEFILE ) + cbExtraBytes ];  
        if( NULL == m_pwfx )  
    ...
    {  
        ...
        ckIn.ckid = mmioFOURCC( 'd', 'p', 'd', 's' );  
        if( 0 != mmioDescend( m_hmmio, &ckIn, &m_ckRiff, MMIO_FINDCHUNK ) )  
            return DXTRACE_ERR( L"mmioDescend", E_FAIL );  
 
        m_nPacketCount = ckIn.cksize / 4;  
 
        m_aDecodedPacketCumulativeBytes = new UINT32[m_nPacketCount];  
 
        // Read the 'dpds' chunk into m_aDPCB.  
        if( mmioRead( m_hmmio, ( HPSTR )m_aDecodedPacketCumulativeBytes,  
            ckIn.cksize ) != ckIn.cksize )  
            return DXTRACE_ERR( L"mmioRead", E_FAIL );  
