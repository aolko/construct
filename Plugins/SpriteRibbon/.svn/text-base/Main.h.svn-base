...
#include "..\..\Common\CommonAceDecl.hpp"
	...
	long aAddPoint(LPVAL params);
	long aSetNumberOfPoints(LPVAL params);
	long aSetThickness(LPVAL params);
	long aSetSegments(LPVAL params);

	long eDistortX(LPVAL params, ExpReturn& ret);
	long eDistortY(LPVAL params, ExpReturn& ret);
	long eDistortU(LPVAL params, ExpReturn& ret);
	long eDistortV(LPVAL params, ExpReturn& ret);

	/////////////////////////

	void setSpriteDistortionAt(int col, int row, cr::point pos);
	void findSpriteRoutines();
	cr::point distortXY(LPVAL params);

	typedef vector<RibbonNode> RibbonNodeList;
	RibbonNodeList nodes;

	float width;
	int maxPoints;
	int columns;

	float step; // step between the points
	float step_offset; // defines where to start
	
	ACROUTINE setDisplacementRealAt;
	ACROUTINE setDistortMapSize;
	ACROUTINE getMeshCols;
	ACROUTINE getMeshRows;
