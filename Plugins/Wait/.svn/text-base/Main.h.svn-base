...
#include "..\..\Common\CommonAceDecl.hpp"
	...
	long cWaitUntil(LPVAL params);
	long cDelayComplete(LPVAL params);
	long cWaitUntilActivated(LPVAL params);
	long cDelay(LPVAL params);

	long aWaitDelay(LPVAL params);
	long aActiveWaitDelay(LPVAL params);
	long aActivateWaitUntil(LPVAL params);
	long aDeactivateWaitUntil(LPVAL params);
	...
	public:
		...
		bool testTime(float time)
		{
			if(endTimer < time)
				return true;
			return false;
		}	
		map<CRunObjType*, set<CRunObject*> > sol;

	private:
		float endTimer;
	};

	map<int, list<data> > memory;
	list<data>::iterator curData; //might change into a stack later incase its possible for nesting to occur!
	map<int, list<data> >::iterator curMemory;
